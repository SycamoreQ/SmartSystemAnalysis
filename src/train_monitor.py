...
gb = HistGradientBoostingClassifier(
    loss="log_loss",
    max_depth=8,         # Slightly deeper for more states
    learning_rate=0.05,
    max_leaf_nodes=31,
    l2_regularization=1.0,
    random_state=42,
    validation_fraction=None, # We use a separate validation set
    class_weight="balanced"   # Use this or sample_weight, not both
)
# gb.fit(Xtr, ytr, sample_weight=sw) # Use sample_weight OR class_weight
gb.fit(Xtr, ytr)
